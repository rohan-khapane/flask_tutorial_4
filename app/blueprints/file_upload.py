...
def csv():
    ...
    if not os.path.exists('Downloads'):  #creation of the new folder using the os module if not exists
        print('creating downloading folder')
        os.makedirs('Downloads')
    filename=f'{uuid.uuid4()}.csv' #created the filename 
    dataframe.to_csv(os.path.join('Downloads',filename)) #conversion of the data frame to csv file 
...
def download(filename):
    print(filename)
    #return f"{filename}"
    df=pd.read_csv(f'Downloads/{filename}')#reading the csv file from the downloads
